"""Tests targeting 100% coverage of cc_sentinel/core.py.
...
class TestLoadConfig:
    ...
    def test_no_file_uses_defaults(self, tmp_path):
        from cc_sentinel.core import load_config, default_config
        cfg = load_config(tmp_path / "nonexistent.env")
...
class TestApplyConfig:
    ...
    def test_all_pricing_keys(self):
        from cc_sentinel.core import _apply_config
        cfg = self._cfg()
...
class TestFmtHelpers:
    def test_fmt_tokens_billions(self):
        from cc_sentinel.core import _fmt_tokens
    ...
    def test_fmt_cost_large(self):
        from cc_sentinel.core import _fmt_cost
    ...
    def test_fmt_duration_hours(self):
        from cc_sentinel.core import _fmt_duration
...
class TestTipsAnalyzerFull:
    def test_no_sessions_returns_placeholder(self):
        from cc_sentinel.core import TipsAnalyzer, default_config
...
class TestCompareAnalyzerFull:
    ...
    def test_spending_increased(self):
        from cc_sentinel.core import CompareAnalyzer, default_config
...
class MyCustomAnalyzer:
    ...
    def analyze(self, sessions, config):
        from cc_sentinel.core import AnalysisResult
...
class TestCommandEntrypoints:
    ...
    def test_run_report(self, tmp_path, capsys):
        from cc_sentinel import core
        cfg = core.Config(data_dir=tmp_path / ".cc-sentinel")
        ...
        assert (tmp_path / ".cc-sentinel" / "reports").is_dir()

    def test_run_hints_shows_settings(self, capsys):
        from cc_sentinel.core import run_hints
        rc = run_hints({})
        assert rc == 0
        out = capsys.readouterr().out
...
class TestRunStopHook:
    ...
    def test_no_matching_jsonl_returns_early(self, tmp_path):
        from cc_sentinel.core import Config, run_stop_hook
        claude_dir = tmp_path / ".claude"
        (claude_dir / "projects").mkdir(parents=True)
        cfg = Config(data_dir=tmp_path / ".cc-sentinel", claude_dir=claude_dir)
        cfg.data_dir.mkdir()
        with patch("cc_sentinel.core.load_config", return_value=cfg):
...
class TestPreToolUseFull:
    ...
    def test_bash_after_non_bash_resets_chain(self, config, capsys):
        from cc_sentinel.core import run_pre_tool_use, _init_live_state, _load_live_state, _save_live_state
        _init_live_state(config)
...
class TestUrlparseExceptionPaths:
    def test_urlparse_exception_in_analyze_session(self, tmp_path):
        ...
        p.write_text(json.dumps(data[0]))
        with patch("cc_sentinel.core.urlparse", side_effect=Exception("boom")):
            summary = analyze_session(p, "proj", default_config())
...
class TestLiveStateFallbacks:
    def test_load_live_state_corrupt_json_returns_defaults(self, config):
        ...
        from cc_sentinel.core import _load_live_state, _live_state_path
    ...
    def test_save_live_state_oserror_silenced(self, tmp_path):
        ...
        from cc_sentinel.core import _save_live_state, Config
